...
import { fadeInWithOpacity, opacityINOut } from "../animations";
...
const Template1 = () => {
    ...
    const generatePDF = async () => {
        const element = resumeRef.current;
        if (!element) {
            toast.info('Unable to capture bcontent at a moment')
            return;
        }
        htmlToImage
            .toPng(element)
            .then((dataUrl) => {
                ...
                // Assuming you want to create a PDF in portrait orientation
                const pdf = new jsPDF({
                    orientation: 'p',
                    unit: 'mm',
                    format: [adWidth, adHeight],
                });

                const aspectRatio = adWidth / adHeight;
                const imgWidth = adWidth;
                const imgHeight = adWidth / aspectRatio;
                const verticalMargin = (adHeight - imgHeight) / 2;

                // Adding the PNG image to the PDF
